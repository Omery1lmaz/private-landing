...
import { AlertCircle, CheckCircle2, Timer, Zap, TrendingUp, Search, Clock, Rocket } from 'lucide-react'
import gsap from 'gsap'
import { ScrollTrigger } from 'gsap/ScrollTrigger'

gsap.registerPlugin(ScrollTrigger)

export default function FastDeliveryProblemSolution() {
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className="text-center mb-20 max-w-3xl mx-auto space-y-6">
                    <div className="animate-split inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/[0.03] border border-white/[0.08] backdrop-blur-xl hover:border-cyan-500/30 transition-all cursor-default">
                        ...
                        <span className="text-[10px] font-black text-cyan-300/60 tracking-[0.3em] uppercase">Hyper-Accelerated Execution</span>
                    </div>
                    <div className="space-y-4">
                        <h2 className="animate-split text-4xl md:text-5xl lg:text-6xl font-bold text-white tracking-[-0.02em]">
                            <span className="opacity-50 blur-[1px]">Beklemek Değil,</span> <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-[0_0_15px_rgba(34,211,238,0.5)]">
                                Bitirmek İçin Buradayız
                            </span>
                        </h2>
                        <p className="animate-split text-white/50 text-lg md:text-xl font-medium">
                            Aylar süren hantal süreçleri çöpe atın; 14 günde gerçek sonuçlar alın.
                        </p>
                    </div>
                </div>
                        ...
                        <div className="relative bg-[#0f0505] border border-red-900/30 rounded-[30px] p-10 overflow-hidden">
                            ...
                            <div className="relative flex flex-col gap-6">
                                <div className="flex items-center gap-3 text-red-500/80">
                                    <AlertCircle size={24} />
                                    <div>
                                        <h3 className="text-2xl font-bold uppercase tracking-widest line-through decoration-red-500/50 decoration-2 leading-none">Geleneksel Süreçler</h3>
                                        <p className="text-[10px] text-red-500/40 font-mono mt-1">(Hantal, Uzun, Verimsiz)</p>
                                    </div>
                                </div>
                                <ul className="space-y-4 font-mono">
                                    {[
                                        "Bitmek bilmeyen toplantılar ve onay süreçleri",
                                        "Aylar süren geliştirme ve test fazları",
                                        "Pazar fırsatlarının kaçmasına neden olan yavaşlık",
                                        "Netleşmeyen hedefler ve sürekli değişen kapsam",
                                        "Yüksek maliyetli ve düşük geri dönüşlü yatırımlar"
                                    ].map((item, i) => (
                                        <li key={i} className="flex items-center gap-3 text-red-300/60 transition-colors group-hover/legacy:text-red-300">
                                            <span className="flex-shrink-0 text-red-500 font-bold animate-pulse">[SLOW]</span>
                                ...
                                <div className="mt-4 pt-4 border-t border-red-900/20 flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-mono text-red-500/60 uppercase tracking-widest">Status: Atalet</span>
                                </div>
                            </div>
                            {/* Glitch Overlay */}
                            ...
                            <div className="relative flex flex-col gap-8">
                                <div className="flex items-center justify-between border-b border-white/5 pb-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-3xl font-bold text-white leading-none">Hyper-Sprint Engine</h3>
                                            <p className="text-[10px] text-cyan-400 font-mono mt-2 tracking-widest uppercase">(14 Gün, Maksimum Odak)</p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid sm:grid-cols-2 gap-4">
                                    {[
                                        { text: "Rapid Prototyping", icon: TrendingUp, desc: "Fikirlerin anında çalışan prototiplere dönüşmesi." },
                                        { text: "Day 1 Deployment", icon: Zap, desc: "İlk günden itibaren canlı ortama geçiş ve test." },
                                        { text: "Continuous Delivery", icon: CheckCircle2, desc: "Her gün yeni özellikler ve iyileştirmelerle ilerleme." },
                                        { text: "Market-Ready MVP", icon: Zap, desc: "Pazara çıkmaya hazır, güçlü ve fonksiyonel ürün." },
                                        { text: "Laser-Focused Dev", icon: Clock, desc: "Sadece hedefe odaklanan, dikkat dağıtmayan geliştirme." }
                                    ].map((item, i) => (
                ...
                <div className="mt-20 text-center animate-split">
                    <p className="text-xl md:text-2xl text-white/40 font-light italic">
                        "En iyi strateji, <span className="text-white opacity-100 font-medium not-italic">hızlıca uygulanan ve piyasada test edilen stratejidir.</span>"
                    </p>
                </div>
