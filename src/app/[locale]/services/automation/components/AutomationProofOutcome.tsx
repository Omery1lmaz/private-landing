...
import { TrendingUp, ShieldCheck, Clock, Layers, CheckCircle2, ArrowRight } from 'lucide-react'

gsap.registerPlugin(ScrollTrigger)

export default function AutomationProofOutcome() {
    const sectionRef = useRef<HTMLElement>(null)

    const outcomes = [
        {
            label: "Verimlilik Artışı",
            title: "ROI & Operasyonel Tasarruf",
            desc: "Otomatize edilen her süreç, doğrudan kârlılığa yansır. İnsan gücü maliyeti düşerken çıktı hızı artar.",
            results: ["Yüksek Yatırım Getirisi", "Düşük İşletme Gideri", "Hızlı Amortisman"],
            visual: "scale",
            icon: TrendingUp,
            color: "cyan"
        },
        {
            label: "Hata Yönetimi",
            title: "Sıfır Hata Güvencesi",
            desc: "Dikkatsizlik veya yorgunluk kaynaklı hatalar tarihe karışır. Sistemler 24/7 aynı hassasiyetle çalışır.",
            results: ["Yüksek Veri Doğruluğu", "Operasyonel Güvenlik", "Kesintisiz Denetim"],
            visual: "focus",
            icon: ShieldCheck,
            color: "blue"
        },
        {
            label: "Zaman Geri Kazanımı",
            title: "Stratejik Zaman Tasarrufu",
            desc: "Ekibiniz angarya işlerden kurtulur. Yaratıcılık ve strateji gerektiren değerli işlere odaklanma imkanı doğar.",
            results: ["Daha Az Mesai", "Odaklanmış Çalışanlar", "Hızlı İş Teslimi"],
            visual: "flow",
            icon: Clock,
            color: "indigo"
        },
        {
            label: "Ölçeklenebilirlik",
            title: "Sınırsız İşleme Kapasitesi",
            desc: "İş yükü 10 katına çıksa bile sistem yavaşlamaz. Otonom yapılar talebe göre anında genişler.",
            results: ["Esneme Kabiliyeti", "Yük Altında Kararlılık", "Geleceğe Hazır Altyapı"],
            visual: "structure",
            icon: Layers,
            color: "purple"
        }
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="text-center mb-24 md:mb-32 space-y-6">
                    ...
                    <p className="text-gray-400 max-w-2xl mx-auto text-lg md:text-xl font-light leading-relaxed">
                        İyi tasarlanmış bir otomasyon sadece maliyeti düşürmez, <br className="hidden md:block" />
                        <span className="text-white/80">iş yapış biçiminizi kökten değiştirir.</span>
                    </p>
                </div>
                                    ...
                                    {item.visual === 'focus' && (
                                        <div className="absolute -bottom-10 -right-10 w-64 h-64 opacity-20 group-hover:opacity-40 transition-opacity duration-1000">
                                            <div className="absolute inset-0 flex items-center justify-center">
                                                <div className="w-48 h-48 rounded-full border border-blue-500/20 animate-ping shadow-[0_0_50px_rgba(59,130,246,0.1)]" />
                                                <div className="w-32 h-32 rounded-full border border-blue-500/30 absolute" />
                                                <div className="w-16 h-16 rounded-full border border-blue-500/40 absolute animate-pulse" />
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div className="absolute -bottom-10 -right-10 w-64 h-64 opacity-20 group-hover:opacity-40 transition-opacity duration-1000">
                                            <div className="grid grid-cols-4 gap-2 absolute inset-0 p-8 rotate-12">
                                                {[...Array(16)].map((_, j) => (
                                                    <div
                                                        key={j}
                                                        className="aspect-square bg-purple-500/20 rounded-lg animate-pulse"
                                        ...
                                        <div className="absolute -bottom-10 -right-10 w-64 h-64 opacity-20 group-hover:opacity-40 transition-opacity duration-1000">
                                            <div className="absolute bottom-12 right-12 flex items-end gap-2 h-48">
                                                {[30, 45, 60, 85, 100].map((h, k) => (
                                                    <div
                                                        key={k}
                                                        className="w-4 bg-gradient-to-t from-cyan-500/40 to-transparent rounded-full transition-all duration-1000 group-hover:h-full"
                ...
                <div className="mt-32 md:mt-40 text-center border-t border-white/5 pt-20">
                    <p className="text-xl md:text-3xl text-white/80 font-light max-w-3xl mx-auto leading-relaxed italic">
                        ...
                        <span className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 not-italic">
                            insan hatasını denklemin dışına çıkarandır."
                        </span>
                    </p>
                </div>
            </div>
