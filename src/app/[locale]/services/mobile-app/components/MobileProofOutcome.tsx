...
import { Zap, MousePointer, LayoutGrid, TrendingUp, CheckCircle2, ArrowRight, Smartphone, Stars, UserCheck } from 'lucide-react'

gsap.registerPlugin(ScrollTrigger)

export default function MobileProofOutcome() {
    const sectionRef = useRef<HTMLElement>(null)

    const outcomes = [
        {
            label: "Kullanıcı Bağlılığı",
            title: "Yüksek Retention",
            desc: "Uygulamanın sadece yüklenmesi değil, her gün aktif olarak kullanılması için tasarlanmış bağımlılık yapmayan, faydalı UX kurguları.",
            results: ["%60+ kullanıcı tutma", "Artan oturum süresi", "Düşük silinme oranı"],
            visual: "flow",
            icon: UserCheck,
            color: "cyan"
        },
        {
            label: "Sıfır Gecikme",
            title: "Native Akıcılık",
            desc: "FPS düşüşleri ve kasmalar olmadan pürüzsüz kaydırma ve geçişler. Kullanıcınız teknolojinin hızını parmaklarında hissetsin.",
            results: ["60 FPS pürüzsüzlük", "0.2s anlık tepki", "Sıfır çökme garantisi"],
            visual: "focus",
            icon: Zap,
            color: "orange"
        },
        {
            label: "App Store Başarısı",
            title: "Market Görünürlüğü",
            desc: "Apple Store ve Google Play store optimizasyonları (ASO) ile uygulamanızın daha fazla organik trafik çekmesini sağlıyoruz.",
            results: ["Yüksek mağaza puanı", "Üst sıralarda yerleşim", "Organik indirme artışı"],
            visual: "structure",
            icon: Stars,
            color: "blue"
        },
        {
            label: "Ölçeklenebilir Büyüme",
            title: "Sınırsız Altyapı",
            desc: "Kullanıcı sayınız binlerden milyonlara çıkarken bile performansını kaybetmeyen, bulut öncelikli backend sistemleri.",
            results: ["Milyonlarca aktif istek", "Veri güvenliği (GDPR/KVKK)", "Global dağıtım"],
            visual: "scale",
            icon: TrendingUp,
            color: "emerald"
        }
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="text-center mb-24 md:mb-32 space-y-6">
                    ...
                    <p className="text-gray-400 max-w-2xl mx-auto text-lg md:text-xl font-light leading-relaxed">
                        Doğru bir mobil uygulama sadece kod değildir, <br className="hidden md:block" />
                        <span className="text-white/80">işinizin parmak uçlarındaki sürdürülebilir başarısıdır.</span>
                    </p>
                </div>
                                    ...
                                    {item.visual === 'focus' && (
                                        <div className="absolute -bottom-10 -right-10 w-64 h-64 opacity-20 group-hover:opacity-40 transition-opacity duration-1000">
                                            <div className="absolute inset-0 flex items-center justify-center">
                                                <div className="w-48 h-48 rounded-full border border-orange-500/20 animate-ping" />
                                                <div className="w-32 h-32 rounded-full border border-orange-500/30 absolute" />
                                                <div className="w-16 h-16 rounded-full border border-orange-500/40 absolute animate-pulse" />
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div className="absolute -bottom-10 -right-10 w-64 h-64 opacity-20 group-hover:opacity-40 transition-opacity duration-1000">
                                            <div className="grid grid-cols-4 gap-2 absolute inset-0 p-8 rotate-12">
                                                {[...Array(16)].map((_, j) => (
                                                    <div
                                                        key={j}
                                                        className="aspect-[3/4] bg-blue-500/20 rounded-lg animate-pulse"
                ...
                <div className="mt-32 md:mt-40 text-center border-t border-white/5 pt-20">
                    <p className="text-xl md:text-3xl text-white/80 font-light max-w-3xl mx-auto leading-relaxed italic">
                        "Uygulama indirilince iş bitmez. <br />
                        <span className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-400 not-italic">
                            Elite altyapı her açılışta marka değerinizi artırır."
                        </span>
                    </p>
                </div>
            </div>
